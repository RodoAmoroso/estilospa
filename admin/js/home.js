var Actions = {
	...
	save:function(type){
		...
		var img = {photoname:$('#th_image_'+type).attr('data-filename'),extension:$('#th_image_'+type).attr('data-extension'),position:$('#th_image_'+type).css('backgroundPosition')};
		...
		ajax('admin/home/get')
			.then(function(DATA){
				$.each(DATA.results,function(k,v){
					...
					mod.find('.thumb').css({backgroundImage:'url('+ROOT+'img/home/'+img.photoname+'-t.'+img.extension+')'});
					if(v.type=='main'){
						$('#main').append(mod);
					}
					if(v.type=='side'){					
	...
	find:function(type){
		...
		ajax('admin/home/find',{ID:Actions.ID})
			.then(function(DATA){
				...
				$('#th_image_'+type).attr({'data-filename':img.photoname,'data-extension':img.extension}).css({backgroundImage:'url('+ROOT+'img/home/'+img.photoname+'-o.'+img.extension+')',backgroundPosition:img.position});
