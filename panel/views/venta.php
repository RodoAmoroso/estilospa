...
<section class="gral-section">
	<div class="container">
		...
		<div class="sale-container">
			...
			<div class="mod-sales" data-id="3">
				<div class="sale-header">
					<div class="thumb-container">
						<div class="thumb thumb-cover" style="background-image: url(<?= $image ?>);"></div>
					</div>
					<div class="caption">
						<h1 data-tag="ordernumber" class="sz-16 fw-400">Orden Nro.: <?=$_sale->collection_id?></h1>
						<h2 data-tag="price" class="sz-14">Precio Unit.: $ <?=number_format($_sale->price,2,',','.')?> | Cant.: <?=$_sale->quantity?> | <span class="fw-400">Total: $ <?=number_format($_sale->price*$_sale->quantity,2,',','.')?></span></h2>
						<h2 data-tag="title" class="sz-11">
							<a href="<?=ROOT.'promo/'.$_sale->permalink.'/'.$_sale->idpromo.'-'.Permalink($_sale->title) ?>" target="_blank" ><?=$_sale->title?></a>
						</h2>
	
						<small data-tag="date" class="cl-gray-40">Fecha de compra: <?=$_sale->fecha?> hs.</small>
						<hr>
						...
						?>
						<div class="sz-9 pad-8 alert-<?=$collection_status->label?>"><?=$collection_status->text?></div>
