...
<section class="gral-section">
	<div class="container">
		...
		<div id="sales" class="well">
			<?php 
			$_SALES->get();
			if(count($_SALES->data())):
				foreach($_SALES->data() as $key=>$sale):
					if(!is_null($sale->title)){
						$img = json_decode($sale->gallery);
						$imgpromo = ROOTPATH.'img/promos/'.$img[0]->photoname.'-t.'.$img[0]->extension;
						$titlepromo = '<a href="'.ROOTPATH.'promo/'.$sale->permalink.'/'.$sale->idpromo.'-'.Permalink($sale->title).'">'.$sale->title.'</a> / <a href="'.ROOTPATH.'centros/'.$sale->permalink.'">'.$sale->clientname.'</a>';
					}else{
						$imgpromo = 'none';
				...
				<div class="sale-footer sale-user-footer">
					<div class="sale-user">
						<div class="feedback" >
							...
							<p><a href="<?= ROOTPATH.'calificar/'.$sale->id ?>" class="btn btn-sm btn-success"><i class="fa fa-thumbs-up"></i> Calificar</a></p>
							<?php else: ?>
