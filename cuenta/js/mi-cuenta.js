...
Actions = {
	...
	get:function(){
		...
		AjaxConnection('jxClients.php',{Mode:'getsales',OrderNumber:$('#fd_ordernumber').val(),From:$('#fd_from').val(),To:$('#fd_to').val()},function(DATA){
			...
			$.each(DATA.Results,function(k,v){
				...
				if(v.title == null){
					...
					mod.find('[data-tag="title"]').html('<a href="'+ROOTPATH+'promo/'+v.permalink+'/'+v.idpromo+'-'+Permalink(v.title)+'">'+v.title+'</a>');
				}
...
$(function(){
	$('#btn_change_plan').click(function(){
		$(this).button('loading');
		AjaxConnection('jxClients.php',{Mode:'changeplan'},function(DATA){
			$('#modal_change_plan .modal-body').html('<p class="alert alert-success">Tu solicitud fue enviada con éxito! En breve nos comunicaremos contigo.</p>');
			$('#btn_change_plan').button('reset').remove();
		});
	});
	Actions.init();
});