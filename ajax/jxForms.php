...
switch (Input::get('Mode')) {
		...
		if($_CLIENTS->get()){
			foreach ($_CLIENTS->data() as $client){
				...
				$_NOTIFICATIONS->add(array(
					...
					'body'=>'<h4>Hola '.$client->name.', '.Input::get('Name').' se ha contactado a través de la página de EstiloSPA para hacer una consulta:</h4><p style="line-height:18pt"><b>Nombre:</b> '.Input::get('Name').'<br /><b>E-mail:</b> '.Input::get('Mail').'<br /><b>Teléfono:</b> '.Input::get('Phone').'<br /></p><p>'.Input::get('Message').'</p>',
	...
	case 'requestpromo':
		...
		$MailBody  = '<h4>Hola '.$_CLIENTS->data()->name.', '.Input::get('Name').' se ha contactado a través de la página de EstiloSPA para consultar acerca de esta promo:</h4>
			<p><a href="'.ROOTPATH.'promo/'.$_CLIENTS->data()->permalink.'/'.$_PROMOS->data()->id.'-'.Permalink($_PROMOS->data()->title).'" >'.$_PROMOS->data()->title.'</a></p>
			<p style="line-height:18pt">
				<b>Nombre:</b> '.Input::get('Name').'<br />
				<b>E-mail:</b> '.Input::get('Mail').'<br />
				<b>Teléfono:</b> '.Input::get('Phone').'<br />
