var Actions = {
	...
	save:function(type){
		...
		var img = {
			f:$('#th_image_'+type).attr('data-filename'),
			e:$('#th_image_'+type).attr('data-extension')
		};
		var link = {
		...
		ajax('admin/home/get')
			.then(function(data){
				$.each(data.results,function(k,v){
					...
					mod.find('.thumb').css({
						backgroundImage:`url(${v.image.small})`
					});
					if(v.type=='main'){
						$('#main').append(mod);
					}
	...
	find:function(type){
		...
		ajax('admin/home/find',{id:Actions.ID})
			.then(function(data){
				...
				$('#th_image_'+type)
				.attr({
					'data-filename':data.result.image.f,
					'data-extension':data.result.image.e
				})
				.css({
					backgroundImage:`url(${data.result.image.big})`
				});
