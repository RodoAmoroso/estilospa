...
Clients.buildvideos = function(IMG,ID){
	var mod = $('#mod_thumb').clone();
	mod.removeClass('dp-none').addClass('dp-ib').removeAttr('id');
...
Clients.save = function(action){
	var logo = {};
	if($('#logo_client').attr('data-filename') == undefined){
		Swal.fire({
			type:'warning',
			text:'Debes subir un logo para tu centro.'
		});
		return false;
	}else{
	...
	//return console.log(idstores);
	ajax('panel/clients/save',{
		Name:$('#fd_name').val(),
...
Clients.find = function(){
	ajax('panel/clients/find')
		.then(function(DATA){
...
Clients.init = function(){
	...
	$('#btn_save,#btn_preview').click(function(){
		CheckFields(['#fd_name','#fd_subtitle','#fd_mail']);
		if($('#fd_name').val() == '' || $('#fd_subtitle').val() == '' || $('#fd_mail').val() == ''){
			//$('#fd_name,#fd_subtitle,#fd_permalink').addClass('required');
			$('a[href="#tab_general"]').trigger('click');
			return;
		}
		$('#fd_name,#fd_subtitle,#fd_mail').removeClass('required');
		...
		if($('#features .mod-list').length==0){
			...
			Swal.fire({
				type:'warning',
				text:'Debes agregar al menos una descripción o característica.'
			});
			return false;
		}
...
var Stores = {
	...
	find:function(){
		ajax('panel/stores/find',{IDS:Stores.ID})
			.then(function(DATA){
				var obj = DATA.result;
	...
	init:function(){
		...
		$('#stores').sortable({
			update:function(){
				...
				ajax('panel/clients/reorder',{ArrID:arr},function(DATA){});
			}
		});
