...
<section class="gral-section qualify">
	<div class="container">
		...
		<div id="promos" class="well">
			...
			<?php 
			if($_FAVS->getpromos($_USER->data()->id)):
				foreach($_FAVS->data() as $fav):					
					$img = json_decode($fav->gallery);
			?>
			<div data-id="<?= $fav->id ?>" class="mod-sales">
				<div class="sale-header">
					<div class="thumb-container">						
						<div class="thumb thumb-cover" style="background-image:url(<?= ROOTPATH.'img/promos/'.$img[0]->photoname.'-t.'.$img[0]->extension ?>)" ></div>
					</div>
					<div class="caption">
						<h1 data-tag="ordernumber" class="sz-16 fw-400"><a href="<?= ROOTPATH.'promo/'.$fav->permalink.'/'.$fav->idpromo.'-'.Permalink($fav->title) ?>"><?= $fav->title ?></a></h1>
						...
						<?php endif; ?>
						<div><?= Stars($_PROMOS->rating($fav->idpromo),'sz-7'); ?></div>
					</div>
		...
		<div id="clients" class="well">
			...
			<?php 
			if($_FAVS->getclients($_USER->data()->id)):
				foreach($_FAVS->data() as $fav):					
			...
			<div data-id="<?= $fav->id ?>" class="mod-sales">			
				<div class="sale-header">
					<div class="thumb-container">						
						<div class="thumb thumb-cover" style="background-image:url(<?= ROOTPATH.'img/clients/'.$img->photoname.'.'.$img->extension ?>)" ></div>
					</div>
					<div class="caption">
						<h1 class="sz-16 fw-400"><a href="<?= ROOTPATH.'centros/'.$fav->permalink ?>"><?= $fav->name ?> </a></h1>
						<h2 class="sz-14"><?= $fav->subtitle ?></h2>
						<?php if($_STORES->get($fav->idclient)): ?>
						...
						<?php endif; ?>						
						<div><?= Stars($_CLIENTS->rating($fav->idclient),'sz-7'); ?></div>
					</div>
					<div data-id="<?= $fav->id ?>" class="close delete">
						<i class="fa fa-times"></i>
					</div>
